...
def exam_(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            All_Exam = Question_Choice.objects.filter(Q(Class_Choice=class_choice_) & Q(Lessons_Name=Lessons_choice_) & Q(Period_Name=period_choice_))
            # for all_exam_ in All_Exam:
            #     if all_exam_.Exam_Name:
            #         All_Exam_Table = Exam_Table.objects.filter(Exam_Name=all_exam_.Exam_Name)
            #         for all_exam_table_ in All_Exam_Table:
            #             print("GELEN SINAV ADI:>",all_exam_table_.Exam_Name)
