...
def forum_create(request):
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            forum_topics_post = request.POST.get('topic_type')
            text = request.POST.get('text')
            img = request.FILES.get('forum_image')

            if text:  # Eğer text alanı doluysa
                forum_save = FORUM(author=request.user, text=text, topic_type=forum_topics_post, forum_image=img)
                forum_save.save()
                
                # Success message gönder
                messages.success(request, 'Forum başarıyla oluşturuldu.')

                # Formun tekrar gönderilmesini önlemek için yönlendir
                ...
                messages.success(request, 'Text Alanı Boş Geçilemez.')
                return render(request, "forum/forum_create.html", context)



        elif request.method == 'GET':
            # GET isteğini burada işle
            forum_topics = ("Genel", "Tartışma", "Soru-Cevap")
            context = {
                "forum_topics": forum_topics
            }
        ...
        messages.error(request, 'Konu Başlalatmak İçin Oturum Açlmalısınız...')
