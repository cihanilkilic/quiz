...
def get_lessons(request):
    ...
    # lessons = Topic.objects.filter(Classrooms_Name__id=classroom_id)
    
    
    # options = "<option value='' disabled selected>Ders Seçin</option>"
    # topic_options = "<option value='' disabled selected>Konu Seçin</option>"
    # for lesson in lessons:
    #     options += f"<option value='{lesson.Lessons_Name}'>{lesson.Lessons_Name}</option>"
        
    #     topics = Topic.objects.filter(Lessons_Name_id=lesson_id)
    #     for topic in topics:
    #         topic_options += f"<option value='{topic.Topics_Name}'>{topic.Topics_Name}</option>"
            
    
    #     data = {'options': options,'topic_options': topic_options}
...
#         Question_Types = request.POST['Question_Types']
#         Topics_Name = request.POST['Topics_Name']
#         Level = request.POST['Level']
#         # Question_Image = request.POST['Question_Image']

#         if form.is_valid():
#             article = form.save(commit=False)
#             article.author = request.user
#             article.Question = Question
#             article.Classroom = Classroom
#             article.Lessons = Lessons
#             article.Answer = Answer
#             article.Question_Types = Question_Types
#             article.Level = Level
#             article.Topics_Name = Topics_Name
#             # article.Question_Image = Question_Image
#             article.save()

#             #messages.success(request, 'Sorunuz Sisteme Kaydedildi.')
...
def exam_save(request):
    ...
    return JsonResponse(response_data)
# Örneğin views.py dosyasının başında tanımlayabilirsiniz
...
# Başka bir fonksiyondan global_filtere_list değerine erişim örneği
